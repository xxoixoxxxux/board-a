...
public class MemoServiceImpl implements MemoService {
    ...
    public MemoServiceImpl(MemoRepository memoRepository) { // 생성자 주입
        // String Framework가 MemoRepository 인터페이스를 구현한 인스턴스를 배정함
    ...
    public List<Memo> initialize() {
        ...
        IntStream.rangeClosed(1, 10).forEach(i -> {
            MemoEntity memo = MemoEntity.builder().memoText("dschoi : " + i).build();
    ...
    public List<Memo> readList() {
            ...
            List<Memo> result = new ArrayList<>();  // Memo 객체를 원소로 갖는 리스트형 객체
